...
const DEFAULT_PREFIX = `You are VibeCode, a community-built AI coding assistant.`
const AGENT_SDK_VIBECODE_PRESET_PREFIX = `You are VibeCode, a community-built AI coding assistant, running within the Claude Agent SDK.`
const AGENT_SDK_PREFIX = `You are a VibeCode agent, running on the Claude Agent SDK.`

const CLI_SYSPROMPT_PREFIX_VALUES = [
  DEFAULT_PREFIX,
  AGENT_SDK_VIBECODE_PRESET_PREFIX,
  AGENT_SDK_PREFIX,
] as const
...
export function getCLISyspromptPrefix(options?: {
  ...
  if (options?.isNonInteractive) {
    if (options.hasAppendSystemPrompt) {
      return AGENT_SDK_VIBECODE_PRESET_PREFIX
    }
...
function isAttributionHeaderEnabled(): boolean {
  if (isEnvDefinedFalsy(process.env.VIBECODE_ATTRIBUTION_HEADER)) {
    return false
  }
...
export function getAttributionHeader(fingerprint: string): string {
  ...
  const entrypoint = process.env.VIBECODE_ENTRYPOINT ?? 'unknown'
