...
export function getLocalISODate(): string {
  ...
  if (process.env.VIBECODE_OVERRIDE_DATE) {
    return process.env.VIBECODE_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.VIBECODE_OVERRIDE_DATE
    ? new Date(process.env.VIBECODE_OVERRIDE_DATE)
    : new Date()
