 ...
 *   1. VIBECODE_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. VIBECODE_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no VIBECODE_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.VIBECODE_DISABLE_AUTO_MEMORY
  ...
  if (isEnvTruthy(process.env.VIBECODE_SIMPLE)) {
    return false
  }
  if (
 ...
 * Resolution order:
 *   1. VIBECODE_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.vibecode (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.VIBECODE_REMOTE_MEMORY_DIR) {
    return process.env.VIBECODE_REMOTE_MEMORY_DIR
  }
 ...
 *
 * SECURITY: projectSettings (.vibecode/settings.json committed to the repo) is
