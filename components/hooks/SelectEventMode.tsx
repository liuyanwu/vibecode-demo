...
export function SelectEventMode(t0) {
  ...
  if ($[2] !== restrictedByPolicy) {
    t2 = restrictedByPolicy && <Box flexDirection="column"><Text color="suggestion">{figures.info} Hooks Restricted by Policy</Text><Text dimColor={true}>Only hooks from managed settings can run. User-defined hooks from ~/.vibecode/settings.json, .vibecode/settings.json, and .vibecode/settings.local.json are blocked.</Text></Box>;
