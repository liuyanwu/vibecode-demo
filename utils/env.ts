...
export const getGlobalClaudeFile = memoize((): string => {
  ...
  const filename = `.vibecode${fileSuffixForOauthConfig()}.json`
 ...
 * If VIBECODE_HOST_PLATFORM is set to a valid platform value, that overrides
...
export function getHostPlatformForAnalytics(): Platform {
  const override = process.env.VIBECODE_HOST_PLATFORM
