// Scheduler lease lock for .vibecode/scheduled_tasks.json.
//
...
const LOCK_FILE_REL = join('.vibecode', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .vibecode/ doesn't exist yet — create it and retry once. In steady
