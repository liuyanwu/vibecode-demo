...
/**
 * CCR and VibeCode Desktop spawn the CLI with OAuth and should never fall back
 * to the user's ~/.vibecode/settings.json API-key config (apiKeyHelper,
 ...
 * who runs `vibecode` in their terminal with an API key sees every VCD session
 * also use that key — and fail if it's stale/wrong-org.
 */
function isManagedOAuthContext(): boolean {
  return (
    isEnvTruthy(process.env.VIBECODE_REMOTE) ||
    process.env.VIBECODE_ENTRYPOINT === 'vibecode-desktop'
  )
}
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // `vibecode ssh` remote: ANTHROPIC_UNIX_SOCKET tunnels API calls through a
  // local auth-injecting proxy. The launcher sets VIBECODE_OAUTH_TOKEN as a
  // placeholder iff the local side is a subscriber (so the remote includes the
  // oauth-2025 beta header to match what the proxy will inject). The remote's
  // ~/.vibecode settings (apiKeyHelper, settings.env.ANTHROPIC_API_KEY) MUST NOT
  ...
  if (process.env.ANTHROPIC_UNIX_SOCKET) {
    return !!process.env.VIBECODE_OAUTH_TOKEN
  }
  ...
  const hasExternalAuthToken =
    ...
    apiKeyHelper ||
    process.env.VIBECODE_API_KEY_FILE_DESCRIPTOR
...
export function getAuthTokenSource() {
  ...
  if (process.env.VIBECODE_OAUTH_TOKEN) {
    return { source: 'VIBECODE_OAUTH_TOKEN' as const, hasToken: true }
  }
  ...
  if (oauthTokenFromFd) {
    ...
    if (process.env.VIBECODE_OAUTH_TOKEN_FILE_DESCRIPTOR) {
      return {
        source: 'VIBECODE_OAUTH_TOKEN_FILE_DESCRIPTOR' as const,
        hasToken: true,
      }
    }
    return {
      source: 'VCR_OAUTH_TOKEN_FILE' as const,
      hasToken: true,
    }
  }
  ...
  if (shouldUseClaudeAIAuth(oauthTokens?.scopes) && oauthTokens?.accessToken) {
    return { source: 'vibecode.ai' as const, hasToken: true }
  }
...
export function getAnthropicApiKeyWithSource(
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      !apiKeyEnv &&
      !process.env.VIBECODE_OAUTH_TOKEN &&
      !process.env.VIBECODE_OAUTH_TOKEN_FILE_DESCRIPTOR
    ) {
      throw new Error(
        'ANTHROPIC_API_KEY or VIBECODE_OAUTH_TOKEN env var is required',
      )
    }
 ...
 * from ~/.vibecode/settings.json or project settings is ignored.
 */
 ...
 * Uses VIBECODE_API_KEY_HELPER_TTL_MS env var if set and valid,
 * otherwise defaults to 5 minutes
 */
export function calculateApiKeyHelperTTL(): number {
  const envTtl = process.env.VIBECODE_API_KEY_HELPER_TTL_MS
  ...
  if (envTtl) {
    ...
    logForDebugging(
      `Found VIBECODE_API_KEY_HELPER_TTL_MS env var, but it was not a valid number. Got ${envTtl}`,
      { level: 'error' },
    )
  }
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.vibecode.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.vibecode.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.vibecode.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
export function saveOAuthTokensIfNeeded(tokens: OAuthTokens): {
  ...
  try {
    ...
    const existingOauth = storageData.vibecodeAiOauth

    storageData.vibecodeAiOauth = {
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  if (process.env.VIBECODE_OAUTH_TOKEN) {
    ...
    return {
      accessToken: process.env.VIBECODE_OAUTH_TOKEN,
      refreshToken: null,
      expiresAt: null,
  ...
  try {
    const secureStorage = getSecureStorage()
    const storageData = secureStorage.read()
    const oauthData = storageData?.vibecodeAiOauth
...
export function getOtelHeadersFromHelper(): Record<string, string> {
  ...
  const debounceMs = parseInt(
    process.env.VIBECODE_OTEL_HEADERS_HELPER_DEBOUNCE_MS ||
...
export function getAccountInformation() {
  ...
  if (
    authTokenSource === 'VIBECODE_OAUTH_TOKEN' ||
    authTokenSource === 'VIBECODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ) {
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  // in ~/.vibecode.json is user-writable and cannot be trusted.
  const { source } = getAuthTokenSource()
  const isEnvVarToken =
    source === 'VIBECODE_OAUTH_TOKEN' ||
    source === 'VIBECODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ...
  if (isEnvVarToken) {
    const envVarName =
      source === 'VIBECODE_OAUTH_TOKEN'
        ? 'VIBECODE_OAUTH_TOKEN'
        : 'VIBECODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
    return {
      valid: false,
      message:
