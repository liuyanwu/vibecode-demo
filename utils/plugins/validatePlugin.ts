...
function detectManifestType(
  ...
  // Check if it's in .vibecode-plugin directory
  if (dirName === '.vibecode-plugin') {
 ...
 * '..' a user added to "climb out of .vibecode-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .vibecode-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .vibecode-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .vibecode-plugin/), not relative to marketplace.json. ' +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.vibecode-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.vibecode-plugin/plugin.json says "${manifestVersion}". ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .vibecode-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.vibecode-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .vibecode-plugin/marketplace.json or .vibecode-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
