 ...
 * Format: ~/.vibecode/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.vibecode/plugins/cache/{plugin-name}/
 *
...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.VIBECODE_REMOTE)
 ...
 * VIBECODE_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
function resolveGitSubdirUrl(url: string): string {
  if (/^[a-zA-Z0-9-_.]+\/[a-zA-Z0-9-_.]+$/.test(url)) {
    return isEnvTruthy(process.env.VIBECODE_REMOTE)
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.vibecode-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".vibecode-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".vibecode-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.vibecode-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".vibecode-plugin/name")
 ...
 *
 * VIBECODE_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 ...
 * - After modifying .vibecode-plugin/ directory (for export)
