 ...
 * - CLI flag: --cowork
 * - Environment variable: VIBECODE_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via VIBECODE_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable VIBECODE_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.VIBECODE_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. VIBECODE_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.vibecode/plugins or ~/.vibecode/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when VIBECODE_PLUGIN_CACHE_DIR is set via settings.json
  // `env` (not shell), ~ is not expanded by the shell. Without this, a value
  // like "~/.vibecode/plugins" becomes a literal `~` directory created in the
  // cwd of every project (gh-30794 / CC-212).
  const envOverride = process.env.VIBECODE_PLUGIN_CACHE_DIR
  if (envOverride) {
 ...
 * image and point VIBECODE_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.VIBECODE_PLUGIN_SEED_DIR
  if (!raw) return []
