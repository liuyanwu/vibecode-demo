 ...
 *
 * Reads the configuration file at ~/.vibecode/plugins/known_marketplaces.json
 ...
 *     "installLocation": "/Users/me/.vibecode/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.vibecode/plugins/marketplaces/company-plugins",
 ...
 *
 * Writes the configuration to ~/.vibecode/plugins/known_marketplaces.json,
 ...
 *
 * Pulls latest changes with a configurable timeout (default 120s, override via VIBECODE_PLUGIN_GIT_TIMEOUT_MS).
...
function getPluginGitTimeoutMs(): number {
  const envValue = process.env.VIBECODE_PLUGIN_GIT_TIMEOUT_MS
  if (envValue) {
...
function enhanceGitPullErrorMessages(result: {
  ...
  if (result.error?.includes('timed out')) {
    ...
    return {
      ...result,
      stderr: `Git pull timed out after ${timeoutSec}s. Try increasing the timeout via VIBECODE_PLUGIN_GIT_TIMEOUT_MS environment variable.\n\nOriginal error: ${result.stderr}`,
    }
  }
 ...
 *
 * Clones a git repository with a configurable timeout (default 120s, override via VIBECODE_PLUGIN_GIT_TIMEOUT_MS)
...
export async function gitClone(
  ...
  if (result.error?.includes('timed out')) {
    return {
      ...result,
      stderr: `Git clone timed out after ${Math.round(timeoutMs / 1000)}s. The repository may be too large for the current timeout. Set VIBECODE_PLUGIN_GIT_TIMEOUT_MS to increase it (e.g., 300000 for 5 minutes).\n\nOriginal error: ${result.stderr}`,
    }
  }
 ...
 * - GitHub: Clones repo and looks for .vibecode-plugin/marketplace.json
    ...
    switch (source.source) {
      ...
      case 'github': {
        ...
        marketplacePath = join(
          temporaryCachePath,
          source.path || '.vibecode-plugin/marketplace.json',
        )
        break
      }
      ...
      case 'file': {
        ...
        // File sources point to .vibecode-plugin/marketplace.json, so the marketplace
        // root is two directories up (parent of .vibecode-plugin/)
      ...
      case 'directory': {
        // For directories, look for .vibecode-plugin/marketplace.json
        // Resolve to absolute so error messages show the actual path checked
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.vibecode-plugin', 'marketplace.json')
 ...
 * The configuration is saved to ~/.vibecode/plugins/known_marketplaces.json.
 *
...
async function readCachedMarketplace(
  ...
  // For git-sourced directories, the manifest lives at .vibecode-plugin/marketplace.json.
  ...
  const nestedPath = join(installLocation, '.vibecode-plugin', 'marketplace.json')
  try {
  ...
  try {
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      if (source.source === 'github') {
        ...
        if (isEnvTruthy(process.env.VIBECODE_REMOTE)) {
