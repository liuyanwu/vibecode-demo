 ...
 * user-controlled files (~/.vibecode/settings.json and ~/.vibecode.json),
 ...
 *
 * We read from global config (~/.vibecode.json) and user settings
 * (~/.vibecode/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.vibecode/settings.json),
