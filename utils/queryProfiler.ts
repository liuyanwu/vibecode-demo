 ...
 * pipeline from user input to first token arrival. Enable by setting VIBECODE_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.VIBECODE_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set VIBECODE_PROFILE_QUERY=1)'
  }
