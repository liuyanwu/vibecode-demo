...
const TEAMMATE_ENV_VARS = [
  ...
  'VIBECODE_USE_BEDROCK',
  'VIBECODE_USE_VERTEX',
  'VIBECODE_USE_FOUNDRY',
  // Custom API endpoint
  'ANTHROPIC_BASE_URL',
  // Config directory override
  'CLAUDE_CONFIG_DIR',
  // CCR marker — teammates need this for CCR-aware code paths. Auth finds
  // its own way via /home/claude/.vibecode/remote/.oauth_token regardless;
 ...
 * Always includes CLAUDECODE=1 and VIBECODE_EXPERIMENTAL_AGENT_TEAMS=1,
 * plus any provider/config env vars that are set in the current process.
 */
export function buildInheritedEnvVars(): string {
  const envVars = ['CLAUDECODE=1', 'VIBECODE_EXPERIMENTAL_AGENT_TEAMS=1']
