// Scheduled prompts, stored in <project>/.vibecode/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.vibecode', 'scheduled_tasks.json')
...
/**
 * Read and parse .vibecode/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .vibecode/scheduled_tasks.json with the given tasks. Creates .vibecode/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.vibecode'), { recursive: true })
 ...
 * written to .vibecode/scheduled_tasks.json and dies with the process. The
