...
export const findGitBashPath = memoize((): string => {
  if (process.env.VIBECODE_GIT_BASH_PATH) {
    if (checkPathExists(process.env.VIBECODE_GIT_BASH_PATH)) {
      return process.env.VIBECODE_GIT_BASH_PATH
    }
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.error(
      `Claude Code was unable to find VIBECODE_GIT_BASH_PATH path "${process.env.VIBECODE_GIT_BASH_PATH}"`,
    )
  ...
  // biome-ignore lint/suspicious/noConsole:: intentional console output
  console.error(
    'VibeCode on Windows requires git-bash (https://git-scm.com/downloads/win). If installed but not in PATH, set environment variable pointing to your bash.exe, similar to: VIBECODE_GIT_BASH_PATH=C:\\Program Files\\Git\\bin\\bash.exe',
  )
  // eslint-disable-next-line custom-rules/no-process-exit
  process.exit(1)
})
