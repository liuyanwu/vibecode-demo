 ...
 * filesystems (macOS/Windows) like `.vibecode/Settings.locaL.json`.
 *
...
/**
 * If filePath is inside a .vibecode/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .vibecode/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.vibecode', 'skills')),
      prefix: '/.vibecode/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.vibecode', 'skills')),
      prefix: '~/.vibecode/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.vibecode/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.vibecode/./settings.json` which would evade the endsWith() check
  const expandedPath = expandPath(filePath)

  // Normalize for case-insensitive comparison to prevent bypassing security
  // with paths like .vibecode/Settings.locaL.json
  const normalizedPath = normalizeCaseForComparison(expandedPath)

  // Use platform separator so endsWith checks work on both Unix (/) and Windows (\)
  if (
    normalizedPath.endsWith(`${sep}.vibecode${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.vibecode${sep}settings.local.json`)
  ) {
    // Include .vibecode/settings.json even for other projects
    return true
  }
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  // Check if file is within .vibecode/commands or .vibecode/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.vibecode', 'commands')
  const agentsDir = join(getOriginalCwd(), '.vibecode', 'agents')
  const skillsDir = join(getOriginalCwd(), '.vibecode', 'skills')
 ...
 * Path format: ~/.vibecode/projects/{sanitized-cwd}/...
 */
...
// and per-turn from BashTool prompt. Inputs (VIBECODE_TMPDIR env + platform) are
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .vibecode/worktrees/ is a structural path (where VibeCode stores
      // git worktrees), not a user-created dangerous directory. Skip the .vibecode
      // segment when it's followed by 'worktrees'. Any nested .vibecode directories
      ...
      if (dir === '.vibecode') {
        ...
        if (
          ...
          break // Skip this .vibecode, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .vibecode , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .vibecode , .bashrc..., settings.json.
 ...
 * - Claude config files (.vibecode/settings.json, .vibecode/commands/, .vibecode/agents/)
...
export function pathInWorkingPath(path: string, workingPath: string): boolean {
  ...
  // checks on case-insensitive filesystems (macOS/Windows) like .vibecode/CoMmAnDs
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .vibecode/)
    return {
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .vibecode is a dangerous directory
  ...
  // 1.6. Check for .vibecode/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .vibecode/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .vibecode/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.vibecode) rule in userSettings (e.g. from sandbox
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .vibecode/ (project or global).
    // Accepts both the broad patterns ('/.vibecode/**', '~/.vibecode/**') and
    // narrowed ones like '/.vibecode/skills/my-skill/**' so users can grant
    ...
    // '/.vibecode/../**' from leaking this bypass outside .vibecode/.
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .vibecode/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.vibecode/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.vibecode/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.vibecode is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.vibecode/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .vibecode/launch.json — desktop preview config (dev server command + port).
  ...
  // .vibecode/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .vibecode/ only (not ~/.vibecode/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.vibecode', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.vibecode/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.vibecode/teams/) for swarm coordination
