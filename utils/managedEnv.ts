 ...
 * env vars that the remote's ~/.vibecode settings.env MUST NOT clobber (see
 ...
 * VIBECODE_PROVIDER_MANAGED_BY_HOST in spawn env), strip
 * provider-selection / model-default vars from settings-sourced env so a
 * user's ~/.vibecode/settings.json can't redirect requests away from the
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.VIBECODE_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
 ...
 *
 * - userSettings (~/.vibecode/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  if (ccdSpawnEnvKeys === undefined) {
    ccdSpawnEnvKeys =
      process.env.VIBECODE_ENTRYPOINT === 'claude-desktop'
        ? new Set(Object.keys(process.env))
        : null
  }

  // Global config (~/.vibecode.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.vibecode/settings.json env (gh#217). policy/flag
  ...
  // flagSettings env applied. Eligibility reads VIBECODE_USE_BEDROCK,
  ...
  // when VIBECODE_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * VIBECODE_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
