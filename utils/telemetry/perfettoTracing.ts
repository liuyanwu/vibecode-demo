 ...
 *
 * Usage:
 * 1. Enable via VIBECODE_PERFETTO_TRACE=1 or VIBECODE_PERFETTO_TRACE=<path>
 * 2. Optionally set VIBECODE_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run Claude Code normally
 * 4. Trace file is written to ~/.vibecode/traces/trace-<session-id>.json
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.VIBECODE_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    // Start periodic full-trace write if VIBECODE_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.VIBECODE_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
