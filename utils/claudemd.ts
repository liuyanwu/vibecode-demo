 ...
 * 2. User memory (~/.vibecode/CLAUDE.md) - Private global instructions for all projects
 * 3. Project memory (CLAUDE.md, .vibecode/CLAUDE.md, and .vibecode/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - CLAUDE.md, .vibecode/CLAUDE.md, and all .md files in .vibecode/rules/ are checked in each directory for Project memory
 *
...
function isClaudeMdExcluded(filePath: string, type: MemoryType): boolean {
  ...
  const patterns = getInitialSettings().vibecodeMdExcludes
...
/**
 * Processes all .md files in the .vibecode/rules/ directory and its subdirectories
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Process Managed .vibecode/rules/*.md files
    ...
    if (isSettingSourceEnabled('userSettings')) {
      ...
      // Process User ~/.vibecode/rules/*.md files
    ...
    // .vibecode/worktrees/<name>/ from `claude -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like CLAUDE.md and .vibecode/rules/*.md, so the same
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        // Try reading .vibecode/CLAUDE.md (Project)
        const dotClaudePath = join(dir, '.vibecode', 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
        ...
        // Try reading .vibecode/rules/*.md files (Project)
        const rulesDir = join(dir, '.vibecode', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
    ...
    // This is controlled by VIBECODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD and defaults to off
    // Note: we don't check isSettingSourceEnabled('projectSettings') here because --add-dir
    // is an explicit user action and the SDK defaults settingSources to [] when not specified
    if (isEnvTruthy(process.env.VIBECODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        ...
        // Try reading .vibecode/CLAUDE.md from the additional directory
        const dotClaudePath = join(dir, '.vibecode', 'CLAUDE.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .vibecode/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.vibecode', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
...
export async function getManagedAndUserConditionalRules(
  ...
  // Process Managed conditional .vibecode/rules/*.md files
  const managedClaudeRulesDir = getManagedClaudeRulesDir()
  result.push(
  ...
  if (isSettingSourceEnabled('userSettings')) {
    // Process User conditional .vibecode/rules/*.md files
    const userClaudeRulesDir = getUserClaudeRulesDir()
    result.push(
      ...(await processConditionedMdRules(
        targetPath,
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (CLAUDE.md and .vibecode/CLAUDE.md)
  ...
  // Process project unconditional .vibecode/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .vibecode/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
export async function processConditionedMdRules(
  ...
  return conditionedRuleMdFiles.filter(file => {
    ...
    // For Project rules: glob patterns are relative to the directory containing .vibecode
    // For Managed/User rules: glob patterns are relative to the original CWD
    const baseDir =
      type === 'Project'
        ? dirname(dirname(rulesDir)) // Parent of .vibecode
...
/**
 * Check if a file path is a memory file (CLAUDE.md, CLAUDE.local.md, or .vibecode/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // .md files in .vibecode/rules/ directories
  if (
    name.endsWith('.md') &&
    filePath.includes(`${sep}.vibecode${sep}rules${sep}`)
  ) {
    return true
  }
