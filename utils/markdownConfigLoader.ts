 ...
 * stopping there makes the parent project's `.vibecode/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.vibecode/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .vibecode directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.vibecode/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .vibecode/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const vibecodeSubdir = join(current, '.vibecode', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.vibecode', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .vibecode/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .vibecode/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .vibecode/<subdir> content — loading the main
    ...
    if (gitRoot && canonicalRoot && canonicalRoot !== gitRoot) {
      const worktreeSubdir = normalizePathForComparison(
        join(gitRoot, '.vibecode', subdir),
      )
      const worktreeHasSubdir = projectDirs.some(
        dir => normalizePathForComparison(dir) === worktreeSubdir,
      )
      if (!worktreeHasSubdir) {
        const mainVibecodeSubdir = join(canonicalRoot, '.vibecode', subdir)
        if (!projectDirs.includes(mainVibecodeSubdir)) {
          projectDirs.push(mainVibecodeSubdir)
        }
      }
    }
    ...
    // This prevents the same file from appearing multiple times when ~/.vibecode is
 ...
 * 3. Can be explicitly enabled via VIBECODE_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
 ...
 * @param dir Directory (eg. "~/.vibecode/commands")
 * @returns Array of parsed markdown files with metadata
 */
...
> {
  ...
  // - Fallback: native Node.js (when VIBECODE_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.VIBECODE_USE_NATIVE_FILE_SEARCH)
