...
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.vibecode', dir)
    case 'userSettings':
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    ...
    const managedSkillsDir = join(getManagedFilePath(), '.vibecode', 'skills')
    ...
    const [
      ...
      isEnvTruthy(process.env.VIBECODE_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.vibecode', 'skills')
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored — blocks e.g. node_modules/pkg/.vibecode/skills from
