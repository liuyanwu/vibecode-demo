...
export async function authLogin({
  ...
  const envRefreshToken = process.env.VIBECODE_OAUTH_REFRESH_TOKEN
  if (envRefreshToken) {
    const envScopes = process.env.VIBECODE_OAUTH_SCOPES
    if (!envScopes) {
      process.stderr.write(
        'VIBECODE_OAUTH_SCOPES is required when using VIBECODE_OAUTH_REFRESH_TOKEN.\n' +
