 ...
 * - Epoch management: reads worker_epoch from VIBECODE_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (VIBECODE_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * VIBECODE_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    VIBECODE_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.VIBECODE_WORKER_EPOCH
