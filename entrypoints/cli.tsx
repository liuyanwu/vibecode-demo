...
if (process.env.VIBECODE_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.VIBECODE_ABLATION_BASELINE) {
  for (const k of ['VIBECODE_SIMPLE', 'VIBECODE_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'VIBECODE_DISABLE_AUTO_MEMORY', 'VIBECODE_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  // Session management against the ~/.vibecode/sessions/ registry. Flag
