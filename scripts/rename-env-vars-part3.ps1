# VibeCode 环境变量重命名脚本 - 第三部分
# 处理所有剩余的 CLAUDE_CODE_ 引用（注释、字符串、错误消息等）
...
foreach ($file in $allFiles) {
    ...
    # 简单替换所有 CLAUDE_CODE_ 为 VIBECODE_
    if ($content -match 'CLAUDE_CODE_') {
        $content = $content -replace 'CLAUDE_CODE_', 'VIBECODE_'
        $fileModified = $true
    }
...
Write-Host "Environment variable rename part 3 complete!" -ForegroundColor Cyan
