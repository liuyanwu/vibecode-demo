...
export function isCoordinatorMode(): boolean {
  if (feature('COORDINATOR_MODE')) {
    return isEnvTruthy(process.env.VIBECODE_COORDINATOR_MODE)
  }
  return false
}
...
export function matchSessionMode(
  ...
  if (sessionIsCoordinator) {
    process.env.VIBECODE_COORDINATOR_MODE = '1'
  } else {
    delete process.env.VIBECODE_COORDINATOR_MODE
  }
...
export function getCoordinatorUserContext(
  ...
  const workerTools = isEnvTruthy(process.env.VIBECODE_SIMPLE)
...
export function getCoordinatorSystemPrompt(): string {
  const workerCapabilities = isEnvTruthy(process.env.VIBECODE_SIMPLE)
